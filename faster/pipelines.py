...
class FasterPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('faster.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `faster`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM faster WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO faster (date, title, link, content)"
